...
import OrderFormUI from "../ui/OrderForm";
...
const ProductSelector = ({ value, onChange }: any) => {
  ...
  if (selector) {
    ...
    props = (currentArray as any)?.[selector.index]?.props;
  }
                ...
                {filteredProducts.map((p) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between p-2 rounded-lg cursor-pointer transition-colors w-full ${isSelected ? "bg-blue-50 border border-blue-100" : "hover:bg-gray-100 border border-transparent"}`}
                    >
                      <span className="text-sm truncate">{getLocalizedString(p.name || p.title || `Product ${p.id}`)}</span>
...
export const OrderForm: ComponentConfig<PuckProps["OrderForm"]> = {
  ...
  defaultProps: {
    apiBaseUrl: "https://pakisthanidress.dev-inventory.softzino.xyz/api/v1",
    productId: "3",
