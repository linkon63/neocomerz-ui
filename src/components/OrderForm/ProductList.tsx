...
import type { OrderFormProductListProps } from "../../types/components";
import {
  getLocalizedString,
  getSizesArray,
...
export default function ProductList({
  ...
  return (
    <div className="flex-1 p-3 lg:p-6 bg-white flex flex-col justify-start items-start gap-4 border-b lg:border-b-0 lg:border-r border-neutral-200">
      <h3 className="text-zinc-800 text-2xl lg:text-3xl font-bold leading-tight">
        Select Product
      </h3>

      {isLoadingProduct ? (
        <div className="w-full p-4 text-center text-zinc-500 bg-neutral-50 rounded-xl border border-neutral-200">
          Loading products...
        </div>
      ) : productData ? (
        <div className="w-full flex flex-col gap-3">
          {variants.length > 0 ? (
            variants.map((variant) => {
              ...
              const { label: vLabel, value: vValue } =
                getVariantDisplayValues(variant);
              const dName = getLocalizedString(rawProductName);

              const vPrice =
                variant["price"] ??
                (variant["current_pricing"] as Record<string, unknown>)?.[
                  "unit_price"
                ] ??
                (variant["current_pricing"] as Record<string, unknown>)?.[
                  "retail_price"
              ...
              const hasPrice =
                ...
                vPrice !== undefined &&
                String(vPrice).trim() !== "";
              const dPrice = hasPrice
                ? String(vPrice).includes("৳")
                  ? String(vPrice)
                  : `৳${vPrice}`
                : productPrice ?? "৳1499";
              ...
              let sizes = getSizesArray(
                variant["sizes"] || productData?.["sizes"]
              );
              ...
              if (allowedVariants && allowedVariants.length > 0) {
                ...
                if (allowedList.length > 0) {
                  const sizeLabel = getDynamicSizeLabel(
                    variant,
                    productData
                  ).toLowerCase().trim();
                  if (
                    !allowedList.includes(sizeLabel) &&
                    !allowedList.includes("default")
                  ) {
                    sizes = [];
                  }
                }
              ...
              const sizeAxisLabel = getDynamicSizeLabel(variant, productData);

              return (
                <div
                  key={String(variant["id"])}
                  onClick={() => {
                    setSelectedVariantId(variant["id"] as string | number);
                    const nextSizes = getSizesArray(
                      variant["sizes"] || productData?.["sizes"]
                    );
                      ...
                      {sizes.map((size) => {
                        ...
                        return (
                          <button
                            ...
                            className="w-9 h-9 flex items-center justify-center rounded border text-xs font-bold transition-all border-neutral-200 text-zinc-600 hover:border-neutral-400"
                            style={
                              isSizeSelected
                                ? {
              ...
              {(() => {
                ...
                if (allowedVariants && allowedVariants.length > 0) {
                  const allowedList = allowedVariants
                    ...
                    .filter(Boolean);
                  if (allowedList.length > 0) {
                    const sizeLabel = getDynamicSizeLabel(null, productData)
                      .toLowerCase()
                      .trim();
                    if (
                      !allowedList.includes(sizeLabel) &&
                ...
                const sizeAxisLabel = getDynamicSizeLabel(null, productData);
                return (
                  <div
                    className="flex items-center gap-2"
                    onClick={(e) => e.stopPropagation()}
      ...
      {(() => {
        const desc = getLocalizedString(productData?.["description"]);
        return desc ? (
          <p className="w-full mt-2 text-sm text-zinc-600 font-normal leading-relaxed max-h-32 overflow-y-auto pr-2 whitespace-pre-wrap">
            {desc}
          </p>
        ) : null;
      })()}
      ...
      <div className="w-full mt-2 p-4 rounded-xl border border-neutral-100 bg-neutral-50/50 flex items-center justify-between">
        <span className="text-zinc-800 font-semibold text-sm lg:text-base">
          Quantity:
        </span>
