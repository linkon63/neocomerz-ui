...
export default function HelplineV2({ whatsappNumber = "+880 1712-508063" }: HelplineProps) {
  const formattedNumber = whatsappNumber.replace(/[^0-9+]/g, '');
  return (
    <a href={`https://wa.me/${formattedNumber}`} target="_blank" rel="noopener noreferrer"
      aria-label={`WhatsApp: ${whatsappNumber}`}
      className="inline-flex items-center gap-3 px-5 py-3 rounded-2xl bg-[#25D366] hover:bg-[#1DA851] transition-colors shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-green-400">
      <svg width="24" height="24" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M42 24C42 33.9411 33.9411 42 24 42C20.2082 42 16.6903 40.8276 13.789 38.8255L7.63636 40.3636L9.24864 34.3176C7.20139 31.396 6 27.8383 6 24C6 14.0589 14.0589 6 24 6C33.9411 6 42 14.0589 42 24Z" fill="white" fillOpacity="0.3"/>
