...
// v4 — Dark Theme: asymmetric image mosaic + dark glass-style detail cards
      ...
      <section className="pt-16 md:pt-24" style={{ backgroundColor: background }}>
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 lg:px-12">
          <div className="text-center mb-10">
            <h2 className="text-3xl md:text-5xl font-bold" style={{ color: primary }}>
          ...
          {/* Asymmetric mosaic: first card tall, rest in a 2-row grid */}
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {imageCards.map((card, index) => (
              <div
                key={index}
                className={`relative overflow-hidden rounded-2xl group border border-white/10 hover:border-yellow-400/40 transition-colors ${
                  index === 0 ? 'row-span-2 col-span-2 md:col-span-1 h-[400px] md:h-auto' : 'h-[180px] md:h-[220px]'
                }`}
              >
                <div
                  className="absolute inset-0 bg-cover bg-center brightness-90 group-hover:brightness-100 transition-all duration-300 group-hover:scale-105"
      ...
      <section className="py-16" style={{ backgroundColor: '#1e293b' }}>
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 lg:px-12">
                ...
                <ul className="space-y-2">
                  {card.items?.map((item, i) => (
                    <li key={i} className="flex items-start gap-2">
                      <span
                        className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 text-xs font-bold text-black"
          ...
          <div className="mt-12 flex justify-center gap-4 flex-wrap">
            <a
              href={ctaButton.href}
              className="px-10 py-4 rounded-xl text-black text-xl font-extrabold shadow-xl hover:brightness-110 active:scale-95 transition-all"
              style={{ backgroundColor: primary }}
            >
              {ctaButton.text}
            </a>
