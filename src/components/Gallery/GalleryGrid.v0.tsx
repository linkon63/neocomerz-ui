...
export default function GalleryGridV0({
  ...
  return (
    <section className="py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {(title || description) && (
          <div className="mb-10">
            {title && <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">{title}</h2>}
            {description && <p className="text-base md:text-lg text-gray-500">{description}</p>}
          </div>
        )}
        <div className={`grid grid-cols-1 sm:grid-cols-2 ${images.length <= 2 ? 'md:grid-cols-2' : images.length === 3 ? 'md:grid-cols-3' : 'md:grid-cols-4'} gap-3 min-h-[200px]`}>
          {images.slice(0, 9).map((image, index) => {
            ...
            return (
              <div key={index} className={`relative group overflow-hidden rounded-xl bg-gray-100 shadow-sm hover:shadow-md transition-shadow h-full ${isFeatured ? 'md:col-span-2 md:row-span-2' : ''}`}>
