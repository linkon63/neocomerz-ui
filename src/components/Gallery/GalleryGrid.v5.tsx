...
export default function GalleryGridV5({
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {(title || description) && (
          <div className="text-center mb-10">
            {title && <h2 className="text-3xl md:text-4xl font-extrabold text-purple-700 mb-2">{title}</h2>}
          ...
          {images.slice(0, 9).map((image, index) => {
            ...
            return (
              <div key={index} className={`relative group overflow-hidden rounded-3xl bg-gray-100 shadow-lg hover:shadow-2xl transition-all duration-300 h-full ${isFeatured ? 'md:col-span-2 md:row-span-2' : ''}`}>
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-purple-500/0 to-pink-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              </div>
            );
          })}
        </div>
