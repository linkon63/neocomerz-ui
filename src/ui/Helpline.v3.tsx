...
export default function HelplineV3({ whatsappNumber = "+880 1712-508063" }: HelplineProps) {
  const formattedNumber = whatsappNumber.replace(/[^0-9+]/g, '');
  return (
    <a href={`https://wa.me/${formattedNumber}`} target="_blank" rel="noopener noreferrer"
      aria-label={`WhatsApp: ${whatsappNumber}`}
      className="inline-flex items-center gap-3 px-5 py-3 rounded-xl bg-white border-2 border-green-500 hover:bg-green-50 transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-green-400">
      ...
      <div className="flex flex-col items-start leading-tight">
        <span className="text-xs text-gray-500 font-medium">যেকোন প্রয়োজনে WhatsApp করুন</span>
        <span className="text-base font-bold text-green-600">{whatsappNumber}</span>
      </div>
    </a>
  );
}
