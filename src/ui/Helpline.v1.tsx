...
export default function HelplineV1({ whatsappNumber = "+880 1712-508063" }: HelplineProps) {
  ...
  return (
    <a href={`https://wa.me/${formattedNumber}`} target="_blank" rel="noopener noreferrer"
      ...
      className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-green-200 bg-green-50 hover:bg-green-100 transition-colors focus:outline-none focus:ring-2 focus:ring-green-400">
      ...
      <div className="flex flex-col items-start leading-tight">
        <span className="text-xs text-green-600">যেকোন প্রয়োজনে</span>
        <span className="text-sm font-bold text-green-700">{whatsappNumber}</span>
      </div>
    </a>
  );
}
